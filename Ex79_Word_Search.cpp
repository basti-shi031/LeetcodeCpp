...
class Solution {
    ...
    vector<vector<bool>> visited;//先定义一个global布尔类型的2D数组，用来标记访问过的char
public:
    bool exist(vector<vector<char>> &board, string word) {

        for (int i = 0; i < board.size(); ++i) {
            vector<bool> v(board[i].size());
            visited.push_back(v);
        }

        for (int i = 0; i < board.size(); i++) {//遍历char数组,以里面每一个board[i][j]作为dfs的起始点
    ...
    bool dfs(vector<vector<char>> &board, string word, int i, int j, int index) {
        ...
        if (i >= board.size() || i < 0 || j >= board[i].size() || j < 0 || board[i][j] != word.at(index) ||
            visited[i][j])
            return false;
